...
public class FormArrayByConcatenatingSubarraysOfAnotherArray {
    public static boolean canChoose(int[][] groups, int[] nums) {
        ...
        while (gi < n && k + groups[gi].length - 1 < m) {

            //匹配k开始j长度的一段
            int j = 0;
            while (j < groups[gi].length) {
                if (groups[gi][j] == nums[j + k]) {
                    j++;
                } else {
                    break;
                }
            }
            if (j == groups[gi].length) {
                gi++;
                k = j + k;
            } else {
                k++;
    ...
    public static void main(String[] args) {
        System.out.println("ans = " + canChoose(new int[][]{{21, 22, 21, 22, 21, 30}}, new int[]{21, 22, 21, 22, 21, 22, 21, 30}));
    }
}
